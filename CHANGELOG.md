...
- Fixed `isd_generate` writing a single-coefficient optical distortion as a scalar instead of a list, so USGSCSM could not parse it and failed to build the camera. This affected LRO NAC (`lrolrocnac`), Dawn FC (`dawnfc`), and Galileo SSI radial distortion, whose `odtk` returned a single-valued NAIF keyword unchanged; each now always returns a list. The Galileo SSI CSM distortion path is not otherwise exercised, so for it the fix only makes the ISD well-formed. [#736](https://github.com/DOI-USGS/ale/pull/736)
