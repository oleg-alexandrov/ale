...
### Added
- Added an optional order-8 Lagrange interpolation of quaternions to the ISD generation path, matching ISIS and USGSCSM. It is off by default (SLERP is unchanged) and selected with the `rotation_interpolation` prop or the `--rotation_interpolation lagrange` option of `isd_generate`.
